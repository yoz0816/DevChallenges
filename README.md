# DevChallenges Solutions

A collection of my solutions to challenges from [DevChallenges](https://devchallenges.io/), built to practice and improve my frontend development skills.

I use these challenges to strengthen my understanding of **HTML, CSS, responsive design, CSS Grid, Flexbox, JavaScript, and modern frontend development** by recreating real-world interfaces from provided designs.

## Table of Contents

- [DevChallenges Solutions](#devchallenges-solutions)
  - [Table of Contents](#table-of-contents)
  - [About](#about)
  - [Challenges](#challenges)
  - [Technologies](#technologies)
  - [Learning Goals](#learning-goals)
  - [Useful Resources](#useful-resources)
  - [Author](#author)

## About

This repository contains my completed and ongoing solutions to DevChallenges.

Each challenge is organized in its own directory with its source code, assets, and project-specific README.

The main goal of this repository is to document my frontend development progress and build practical experience by turning designs into responsive, accessible, and maintainable websites.

## Challenges

| Challenge                | Live Demo                                                                         | Source Code                                                                                   |
| ------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| **Meet the Team Section** | [Live Demo](https://yoz0816.github.io/DevChallenges/our-team-layout-master/)      | [Source Code](https://github.com/yoz0816/DevChallenges/tree/main/our-team-layout-master)      |
| **Pricing Table**         |[Live Demo](https://yoz0816.github.io/DevChallenges/pricing-table-master/)        | [Source Code](https://github.com/yoz0816/DevChallenges/tree/main/pricing-table-master)        |
| **Device Shop Checkout**  |[Live Demo](https://yoz0816.github.io/DevChallenges/device-shop-checkout-master/) | [Source Code](https://github.com/yoz0816/DevChallenges/tree/main/device-shop-checkout-master) |
|  **NewsLetter** |[Live Demo](https://yoz0816.github.io/DevChallenges/newsletter/)  | [Source Code](https://github.com/yoz0816/DevChallenges/tree/main/newsletter)  |


## Technologies

The technologies used across this repository include:

* HTML5
* CSS3
* Responsive Design
* Media Queries
* CSS Grid
* Flexbox
* JavaScript
* Git & GitHub
* GitHub Pages

Technologies may vary depending on the requirements of each challenge.

## Learning Goals

Through these challenges, I am focusing on improving my ability to:

* Convert designs into responsive websites
* Build layouts with CSS Grid and Flexbox
* Write clean and maintainable HTML and CSS
* Create responsive interfaces for different screen sizes
* Improve accessibility and semantic HTML
* Work with typography, spacing, colors, and visual hierarchy
* Organize frontend projects professionally
* Use Git and GitHub effectively
* Deploy projects using GitHub Pages
* Build consistency through regular frontend practice

## Useful Resources

* [DevChallenges](https://devchallenges.io/) — Frontend development challenges and practice projects.
* [MDN Web Docs](https://developer.mozilla.org/) — Documentation and references for HTML, CSS, and JavaScript.
* [CSS-Tricks](https://css-tricks.com/) — Articles and guides about CSS and frontend development.

## Author

**Yo Z**

* GitHub: [@yoz0816](https://github.com/yoz0816)
* DevChallenges: [@yoz0816](https://devchallenges.io/)
